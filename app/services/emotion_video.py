...
def init_fer() -> None:
    ...
    # it cannot be caught as a Python exception. Skip the import on Linux CPUs
    # that explicitly lack AVX. On non-Linux (macOS, Windows) /proc/cpuinfo
    # doesn't exist; AVX is not a concern there, so proceed.
    try:
        cpu_flags = ""
        ...
        pass  # not Linux — AVX check not applicable, proceed
